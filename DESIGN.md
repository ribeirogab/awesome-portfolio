...
- **Hero mosaic:** An optional photo grid between the role line and the intro, full column width, 28px above the intro. Three columns of 124px rows (92px on mobile) with 10px gaps; each photo is a 10px-radius tile with a hairline border on the card surface and `object-fit: cover`. Photos repeat in modules of five: the first spans two columns and two rows, the next two stack beside it, the fourth is a single tile and the fifth spans two columns. A trailing module closes its last row: one leftover photo becomes a full-width band two rows tall, two leftovers become a wide tile plus a single, and four leftovers end with a full-width band. No captions, no links, no hover motion. In dark mode photos dim to 92% brightness instead of inverting.
...
- **Dock:** A floating pill (fully rounded, 68px tall) fixed at the bottom center, with a translucent frosted-glass background (14px blur), hairline border, and the deepest shadow in the system. Its popover menus share the frosted treatment with 16px rounded corners and 10px rounded items. When the site has more than one Locale, a globe button sits after the theme toggle and opens a language menu: one row per Locale with its native name and its uppercase code in the index slot; the current Locale is marked `aria-current` and reads in the ink tier.
